...
| client/web/build             | Contains the distributable (or output) from your TypeScript build for client side, e.g. index.html, css, images, and packed Javascript.      |
| client/web/node_modules      | Contains all your npm dependencies for client side.                                                                                          |
| client/web/public            | Static assets that will be used client side, they will be moved to client/web/build folder when client building.                                 |
| **client/web/src**           | Typescript source code for the client React app.                                                                                             |
| **client/web/src/actions**   | Redux actions implementations.                                                                                                               |
| **client/web/src/components**| React components which will be used in pages.                                                                                                |
| **client/web/src/models**    | Typescript definitions. It contains 1. models used **across client & server** and 2. Redux app states & action creators definition.          |
| **client/web/src/pages**     | React components which usually be destination for a **client route** path.                                                                   |
| **client/web/src/reducers**  | Redux reducers implementations.                                                                                                              |
| **client/web/src/shared**    | Common utilities for client.                                                                                                                 |
| **client/web/src/App.tsx**   | Client's main routers, contains paths to pages.                                                                                              |
| **client/web/src/index.tsx** | Client's entry point, i.e. the entry point of React app.                                                                                     |
| client/web/src/react-app-env.d.ts | create-react-app generated file.                                                                                                        |
| client/web/src/serviceWorker.ts   | create-react-app generated file, which is used for debug.                                                                               |
| client/web/src/setupProxy.js | Bypasses all REST API calls from serviceWorker to nodejs server we are using.                                                                |
| **client/web/src/store.ts**  | Redux store configurations, apply middleware such as Thunk.                                                                                  |
| client/web/env.development   | Defines environment variables for **client** build under ```NODE_ENV``` is ```development```.                                                |
| client/web/env.production    | Defines environment variables for **client** build under ```NODE_ENV``` is ```production```.                                                 |
| client/web/package.json      | File that contains npm dependencies as well as [build scripts](#what-if-a-library-isnt-on-definitelytyped) which can only run for client.    |
| client/web/tsconfig.json     | Config settings for compiling **client** code written in TypeScript.                                                                         |
| client/web/yarn.lock         | Yarn stores exactly which versions of each dependency were installed. Generated after you run ```yarn``` or ```yarn install``` in client.    |
...
In production build environment, ```react-scripts```, the tool for use with ```Create React App``` will compile all of your (Typescript) source code and resources into bundles and assets in ```client/web/build``` folder.

For all routes except those for REST APIs, the Node.js server can response ```client/web/build/static/index.html```.
...
This server is implemented in ```client/web/src/serviceWorker.ts```.
...
4. In the file ```client/web/src/setupProxy.js```, we configure that uri starts with ```/api```, ```/auth```, and ```oauth``` should be proxied to port 3001., i.e. the Node.js server will handle these requests forwarded by the serviceWorker.
...
In ```client/web/src/models/UnifiedModel.d.ts```, we define a interface named UnifiedModel.
...
1. Add a interface named ```User``` in ```client/web/src/models```, and extends from ```UnifiedModel```.
2. Fill the ```User``` with fields you desire such as name and address, etc. See what we did in ```client/web/src/models/User.d.ts```.
...
2. For all other request url, return the ```client/web/build/static/index.html```.
...
In the file ```client/web/src/App.tsx```, react-router 4.0+ shows how this works can be done elegantly.
...
3. Since the file ```client/web/build/static/index.html``` would not change unless you re-deploy you app, it's easily be cached among Internet.
4. The React app handles navigation locally, users feel no latency when they navigate back and forth in the browser.

## Styling

In this project, there is no separated file like ```client/web/public/css/main.css```.
...
| ------------------------- | -------------------------------------------------------------------------------------------------- |
| **`install`**             | Install all dependencies defined in `package.json` and `client/web/package.json`.                                |
| `install-server`          | Install all dependencies defined in `package.json`.                  |
| `install-client`          | Install all dependencies defined in `client/web/package.json`.           |
